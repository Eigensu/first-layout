...
async def resolve_contest(
    contest_id: Optional[str], contest_code: Optional[str]
) -> Optional[Contest]:
    if contest_id:
        try:
...
async def add_allowed_team(
    contest: Contest, team_code: str, dry_run: bool = False
) -> bool:
    ...
    if team_code in contest.allowed_teams:
        print(
            f"[SKIP] Team '{team_code}' already allowed for contest {contest.code} ({contest.id})"
        )
        return False

    print(
        f"[PLAN] Add team '{team_code}' to allowed_teams for contest {contest.code} ({contest.id})"
    )
    if dry_run:
        return True
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not contest:
            raise SystemExit(
                "Contest not found. Provide a valid --contest-id or --contest-code"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add an allowed team to a daily contest"
    )
    ...
    parser.add_argument(
        "--team",
        required=True,
        help="Real-world team code or name to allow (e.g., IND, AUS)",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Plan changes without writing"
    )
